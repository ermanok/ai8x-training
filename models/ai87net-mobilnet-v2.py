...
class AI87MobileNetV2(nn.Module):
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        for setting in bottleneck_settings:
            self._create_bottleneck_stage(setting, bias, **kwargs)

        self.post_stage = ai8x.FusedConv2dBNReLU(bottleneck_settings[-1][2], last_layer_width, 1,
                                                 padding=0, stride=1, bias=bias, **kwargs)

        self.classifier = ai8x.FusedAvgPoolConv2d(last_layer_width, num_classes, 1, padding=0,
                                                  stride=1, pool_size=avg_pool_size,
                                                  pool_stride=avg_pool_size, bias=bias, wide=True,
                                                  **kwargs)

    def _create_bottleneck_stage(self, setting, bias, **kwargs):
        ...
        if setting[0] > 0:
            stage.append(ai8x_blocks.Bottleneck(in_channels=setting[1],
            ...
            for _ in range(1, setting[0]):
                stage.append(ai8x_blocks.Bottleneck(in_channels=setting[2],
                                                    out_channels=setting[2],
                                                    stride=1,
                                                    expansion_factor=setting[4],
                                                    bias=bias, **kwargs))
...
def ai87netmobilenetv2cifar100(pretrained=False, **kwargs):
    ...
    return AI87MobileNetV2(pre_layer_stride=1, bottleneck_settings=bottleneck_settings,
                           last_layer_width=1024, avg_pool_size=4, **kwargs)
