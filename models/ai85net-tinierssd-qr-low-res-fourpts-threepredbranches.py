...
class TinySSDBase(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.fire1 = ai8x.FusedConv2dReLU(3, 32, 3, padding=1, bias=False)
        self.fire2 = ai8x.FusedConv2dReLU(32, 32, 3, padding=1, bias=False)

        self.fire3 = ai8x.FusedMaxPoolConv2dReLU(32, 64, 3, padding=1, bias=False)
        self.fire3_1 = ai8x.FusedMaxPoolConv2dReLU(64, 64, 3, padding=1, bias=False)
        self.fire4 = ai8x.FusedConv2dReLU(64, 64, 3, padding=1, bias=False)

        self.fire5 = ai8x.FusedMaxPoolConv2dReLU(64, 64, 3, padding=1, bias=False)
        ...
        self.fire9 = ai8x.FusedMaxPoolConv2dBNReLU(32, 32, 3, padding=1, **kwargs)
    ...
    def forward(self, image):
        ...
        out = self.fire3_1(out)  # (N, 64, 37, 37)
...
class PredictionConvolutions(nn.Module):
    ...
    def __init__(self, n_classes, **kwargs):
        ...
        # Keypoint prediction is added to these layers
    ...
    def forward(self, fire4_feats, fire8_feats, fire9_feats, fire10_feats, conv12_2_feats):
        ...
        k_fire8 = self.kpts_fire8(fire8_feats)
        k_fire8 = k_fire8.permute(0, 2, 3, 1).contiguous()
        k_fire8 = k_fire8.view(batch_size, -1, (8))

        k_fire9 = self.kpts_fire9(fire9_feats)
        k_fire9 = k_fire9.permute(0, 2, 3, 1).contiguous()
        k_fire9 = k_fire9.view(batch_size, -1, (8))

        k_fire10 = self.kpts_fire10(fire10_feats)
        k_fire10 = k_fire10.permute(0, 2, 3, 1).contiguous()
        k_fire10 = k_fire10.view(batch_size, -1, (8))
        ...
        kpts = torch.cat([k_fire8, k_fire9, k_fire10, k_conv12_2], dim=1)
...
class TinierSSDQR(nn.Module):
    ...
    def create_prior_boxes(aspect_ratios=default_aspect_ratios, device='cpu'):
        ...
        if True in (len(aspect_ratios_list) !=
                    len(TinierSSDQR.default_aspect_ratios[0])
        ...
        prior_boxes = torch.FloatTensor(prior_boxes).to(device)  # (1700, 4)
...
def ai85tinierssdqr_lowres_4kpts_3predbrnches(pretrained=False, **kwargs):
    ...
    return TinierSSDQR(aspect_ratios=TinierSSDQR.default_aspect_ratios, **kwargs)


models = [
    {
        'name': 'ai85tinierssdqr_lowres_4kpts_3predbrnches',
        'min_input': 1,
        'dim': 2,
    }
]
